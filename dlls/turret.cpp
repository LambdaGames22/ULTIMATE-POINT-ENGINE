...
void CBaseTurret::Precache( )
{
	...
	PRECACHE_SOUND ("turret/tu_retract.wav"); // just use deploy sound to save memory LOL PSYCH
...
void CBaseTurret :: TraceAttack( entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int bitsDamageType)
{
	...
	if ( !pev->takedamage )
		return;
	UTIL_Ricochet( ptr->vecEndPos, 0.01 );
...
void CSentry::Precache()
{
	...
	m_iBodyGibs = PRECACHE_MODEL( "models/computergibs.mdl" );
}
...
void CSentry::Shoot(Vector &vecSrc, Vector &vecDirToEnemy)
{
	...
	switch(RANDOM_LONG(0,2))
	{
	case 0: EMIT_SOUND(ENT(pev), CHAN_AUTO, "weapons/hks1.wav", 1, ATTN_NORM); break;
	case 1: EMIT_SOUND(ENT(pev), CHAN_AUTO, "weapons/hks2.wav", 1, ATTN_NORM); break;
	case 2: EMIT_SOUND(ENT(pev), CHAN_AUTO, "weapons/hks3.wav", 1, ATTN_NORM); break;
	}
	pev->effects = pev->effects | EF_MUZZLEFLASH;
}
...
void CSentry ::	SentryDeath( void )
{
	...
	if (m_fSequenceFinished && pev->dmgtime + 5 < gpGlobals->time)
	{
		...
		EMIT_SOUND(ENT(pev), CHAN_ITEM, "debris/beamstart15.wav", 1.00, ATTN_NORM ); // 0.85
		...
		RadiusDamage ( pev, pev, 75, CLASS_NONE, DMG_BLAST ); // Step4enko
	}
}

// Step4enko
void CSentry :: ThrowTurretGibs( void )
{
    TraceResult    tr;
    BOOL    gibbed = FALSE;
    ...
    {
        case 0: EMIT_SOUND(ENT(pev), CHAN_WEAPON, "debris/metal1.wav", 1, ATTN_NORM); break;
        case 1: EMIT_SOUND(ENT(pev), CHAN_WEAPON, "debris/metal2.wav", 1, ATTN_NORM); break;
        case 2: EMIT_SOUND(ENT(pev), CHAN_WEAPON, "debris/metal3.wav", 1, ATTN_NORM); break;
        case 3: EMIT_SOUND(ENT(pev), CHAN_WEAPON, "debris/metal4.wav", 1, ATTN_NORM); break;
        case 4: EMIT_SOUND(ENT(pev), CHAN_WEAPON, "debris/metal5.wav", 1, ATTN_NORM); break;
        case 5: EMIT_SOUND(ENT(pev), CHAN_WEAPON, "debris/metal6.wav", 1, ATTN_NORM); break;
    }
		...
		MESSAGE_BEGIN( MSG_PVS, SVC_TEMPENTITY, vecSpot );
			...
			// # of shards
			WRITE_BYTE( 35 );	// let client decide (4)

			// duration
			WRITE_BYTE( 40 );// 3.0 seconds
...
// rand code after this jonny

class CSentryRandom : public CSentry
{
public:
	void Spawn( void );
	void Precache( void );
};
LINK_ENTITY_TO_CLASS( monster_sentry_random, CSentryRandom );

void CSentryRandom :: Spawn( void )
{
	switch (RANDOM_LONG(0,2))
	{
	case 0: CSentry::Spawn(); pev->angles.y = RANDOM_LONG(0,360); break;
	case 1:  break;
	case 2:  break;
	}
}

void CSentryRandom :: Precache( void )
{
	CSentry::Precache();
}
//have i just done it? oh my god i wish please for fuck sake.
