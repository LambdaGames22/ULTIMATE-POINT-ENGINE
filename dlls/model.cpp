...
TYPEDESCRIPTION CEnvModel::m_SaveData[] =
{
	...
	DEFINE_FIELD( CEnvModel, m_fScale, FIELD_FLOAT ), // Step4enko
};
...
void CEnvModel::KeyValue( KeyValueData *pkvd )
{
	...
	else if (FStrEq(pkvd->szKeyName, "m_fScale"))
	{
		m_fScale = atoi(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
	else
