...
void CShotgun::PrimaryAttack()
{
	...
	// Step4enko: Player "shoot" animation.
...
void CShotgun::Reload( void )
{
	...
	if (m_fInSpecialReload == 0)
	{
		...
		m_pPlayer->m_flNextAttack = UTIL_WeaponTimeBase() + 0.2; // 0.6
		m_flTimeWeaponIdle = UTIL_WeaponTimeBase() + 0.2; // 0.6
		m_flNextPrimaryAttack = GetNextAttackDelay(0.4); // 1.0
		m_flNextSecondaryAttack = UTIL_WeaponTimeBase() + 0.4; // 1.0
		return;
	}
...
#ifndef CLIENT_DLL
		// Step4enko
		//UTIL_ScreenShake( pev->origin, 2.5, 3.0, 0.5, 2.5 );
#endif

		SendWeaponAnim( SHOTGUN_RELOAD );

		m_flNextReload = UTIL_WeaponTimeBase() + 0.25; // 0.5
		m_flTimeWeaponIdle = UTIL_WeaponTimeBase() + 0.25; // 0.5
	}
	else
	{
...
void CShotgun::WeaponTick()
{
	if( m_flPumpTime && m_flPumpTime < gpGlobals->time )
	{
	    ...
	    Vector	vecShellVelocity = gpGlobals->v_right * RANDOM_FLOAT(40,90) + gpGlobals->v_up * RANDOM_FLOAT(75,200) + gpGlobals->v_forward * RANDOM_FLOAT(-40, 40);//eject buckshot rounds from shotgun jonny
	    EjectBrass ( pev->origin + gpGlobals->v_up * 4 + gpGlobals->v_forward * 12, vecShellVelocity, pev->angles.y, m_iShell, TE_BOUNCE_SHOTSHELL); //eject buckshot rounds from shotgun jonny
		m_flPumpTime = 0;
	}
}
