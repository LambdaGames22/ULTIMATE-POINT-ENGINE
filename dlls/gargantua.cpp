...
//=========================================================================
// Step4enko Changes:
// Custom model in J.A.C.K.
// Custom health Value in J.A.C.K.
// Added DLIGHT from fire
// DLIGHT from Stomp attack
// DLIGHT from eye
//=========================================================================
...
//#define GARG_AE_BEAM_ATTACK_RIGHT ( 2 )  // No longer used // Step4enko: heyyy lemme check it
#define GARG_AE_LEFT_FOOT		   ( 3 )
#define GARG_AE_RIGHT_FOOT		   ( 4 )
#define GARG_AE_STOMP			   ( 5 )
#define GARG_AE_BREATHE			   ( 6 )
#define GARG_AE_EAT_ATTACK         ( 7 ) // Step4enko: Custom event number ( 7 )
...
void CStomp :: DLight( void ) // Step4enko: DLIGHT system
{
	Vector vecSrc = pev->origin + gpGlobals->v_forward * 2;
		MESSAGE_BEGIN( MSG_PVS, SVC_TEMPENTITY, vecSrc );
			...
			WRITE_BYTE( RANDOM_LONG(12, 24) );		// radius * 0.1 used to be WRITE_BYTE 12
			WRITE_BYTE( 251 );		// r
			WRITE_BYTE( 255 );		// g
			WRITE_BYTE( 158 );		// b
...
void CStomp::Think( void )
{
	...
	if ( tr.pHit && tr.pHit != pev->owner ) // If hit someone
	{
...
Task_t	tlGargFlame[] =
{
	...
	// { TASK_PLAY_SEQUENCE,		(float)ACT_SIGNAL1	}, // Unused "deploy" animation
...
void CGargantua::EyeUpdate( void )
{
	if ( m_pEyeGlow )
	{
		...
		MESSAGE_BEGIN( MSG_PVS, SVC_TEMPENTITY, pev->origin );
			...
			WRITE_BYTE( 4 );		// radius * 0.1 used to be WRITE_BYTE 12
			WRITE_BYTE( 200 );		// r
			WRITE_BYTE( 11 );		// g
			WRITE_BYTE( 11 );		// b
...
void CGargantua::EatAttack( void )
{
...
void CGargantua :: DLight( void ) // Step4enko: DLIGHT system
{
	Vector vecSrc = pev->origin + gpGlobals->v_forward * 2;
		MESSAGE_BEGIN( MSG_PVS, SVC_TEMPENTITY, vecSrc );
			WRITE_BYTE(TE_DLIGHT);
...
void CGargantua::TraceAttack( entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int bitsDamageType )
{
	...
	// UNDONE: Hit group specific damage? // Step4enko: Hey! This is idea...
...
//				EMIT_SOUND_DYN( ENT(pev), CHAN_BODY, pRicSounds[ RANDOM_LONG(0,ARRAYSIZE(pRicSounds)-1) ], 1.0, ATTN_NORM, 0, PITCH_NORM ); // Step4enko: so valve planned enable ric sounds?
		}
		flDamage = 0;
	}
...
void CGargantua::HandleAnimEvent(MonsterEvent_t *pEvent)
{
  ...
  // 	case GARG_AE_EAT_ATTACK:
	//	CBaseEntity *pHurt = CheckTraceHullAttack( 70, 10000000000, DMG_SLASH ); // DMG 10000000000
		//EMIT_SOUND_DYN ( edict(), CHAN_BODY, pFootSounds[ RANDOM_LONG(0,ARRAYSIZE(pFootSounds)-1) ], 1.0, ATTN_GARG, 0, PITCH_NORM + RANDOM_LONG(-10,10) );
		//break;
