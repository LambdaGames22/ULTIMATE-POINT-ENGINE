...
extern "C" char PM_FindTextureType( char *name ); // Step4enko
...
void CBaseMonster :: StartMonster ( void )
{
	...
	if ( pev->movetype != MOVETYPE_FLY && !FBitSet( pev->spawnflags, SF_MONSTER_FALL_TO_GROUND ) )
	{
		...
		{
			ALERT(at_console, "%s \"%s\" stuck in wall--level design error\n", STRING(pev->classname), STRING(pev->targetname));
	...
	{
	//	StartPatrol(UTIL_FindEntityByTargetname( NULL, STRING( pev->target ))); // Step4enko: UNDONE
	}
...
void CBaseMonster :: MonsterInitDead( void )
{
	...
	// Step4enko: Stop to copy health
	pev->max_health		= 8; // pev->health
