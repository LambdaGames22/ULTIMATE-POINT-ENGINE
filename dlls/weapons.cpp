...
#endif
#define anglestorad (2 * M_PI / 360)

		  // Step4enko: Advanced physics system for weapons.
          if ( tr.flFraction < 1.0 ) // TEST TEST: 1.5?
          {
               Vector angdir = Vector(
                    cos(pev->angles.y * anglestorad) * cos(pev->angles.x * anglestorad),
                    sin(pev->angles.y * anglestorad) * cos(pev->angles.x * anglestorad),
                    -sin(pev->angles.x * anglestorad));
               Vector angdiry = Vector(
                    sin(pev->angles.y * anglestorad) * cos(pev->angles.x * anglestorad),
                    cos(pev->angles.y * anglestorad) * cos(pev->angles.x * anglestorad),                     -sin(pev->angles.x * anglestorad));
...
/* // Step4enko: No need to be enabled anymore. Because it will broke holster system.
#if defined( CLIENT_WEAPONS )
