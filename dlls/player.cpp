...
TYPEDESCRIPTION	CBasePlayer::m_playerSaveData[] = 
{
	...
	DEFINE_FIELD( CBasePlayer, m_pNextItem, FIELD_CLASSPTR ), // Step4enko
...
int gmsgTeamNames = 0;
int gmsgImpact = 0; // Step4enko
int gmsgWaterSplash = 0; // Step4enko
...
void LinkUserMessages( void )
{
    ...
    gmsgImpact = REG_USER_MSG( "Impact", -1); // Step4enko
    gmsgWaterSplash = REG_USER_MSG( "WaterSplash", 6 ); // Step4enko
}
...
// fallpain sound, and damage will be inflicted based on how far the player fell
	...
	if ( (FBitSet(pev->flags, FL_ONGROUND)) && (pev->health > 0) && m_flFallVelocity >= PLAYER_FALL_PUNCH_THRESHHOLD )
	{
		...
		// Step4enko: UNDONE: Big water splash effect???
		if (pev->watertype == CONTENT_WATER)
		{
			...
			//{
			    ...
			    EMIT_SOUND(ENT(pev), CHAN_STATIC, "player/water_splash.wav", 1, ATTN_NORM);
				//BigWaterSplash( pev->origin, pev->origin );
...
//inline int FNullEnt( CBaseEntity *ent ) { return (ent == NULL) || FNullEnt( ent->edict() ); } // Step4enko: already moved to cbase.h
...
// Copied from spawn() for now
	...
	// Step4enko: Fixed bug with reload after save/restore.
	TabulateAmmo();
...
void CBasePlayer::CheatImpulseCommands( int iImpulse )
{
	...
	case 101:
		...
		GiveNamedItem( "weapon_deagle" ); // Step4enko
		GiveNamedItem( "item_longjump" ); // Step4enko
#endif
