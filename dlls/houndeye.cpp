...
#define		HOUND_AE_IDLE				0
#define		HOUND_AE_BORED				1
...
void CHoundeye :: SetActivity ( Activity NewActivity )
{
	...
	{
	case ACT_WALK:
		if ( pev->health <= HOUNDEYE_LIMP_HEALTH )
		{
			// Step4enko: limp walk
...
void CHoundeye :: Precache()
{
	...
	PRECACHE_SOUND("houndeye/he_attack2.wav"); // Step4enko
...
void CHoundeye :: SonicAttack ( void )
{
    ...
    MESSAGE_BEGIN( MSG_PVS, SVC_TEMPENTITY, vecSrc2 );
            ...
            WRITE_BYTE( RANDOM_LONG(40,50) );		// radius * 0.1 used to be WRITE_BYTE 12
            WRITE_BYTE( RANDOM_LONG(50,60) );		// r
            WRITE_BYTE( RANDOM_LONG(50,60) );		// g
            WRITE_BYTE( RANDOM_LONG(245, 255) );		// b
	...
	case TASK_HOUND_FALL_ASLEEP:
		{
			...
			pev->skin = 0; // Step4enko: close eye while sleeping
			m_fDontBlink = TRUE; 
			break;
		}
